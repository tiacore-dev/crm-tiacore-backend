...
async def test_edit_company(test_app: AsyncClient, jwt_token_admin, seed_company):
    ...
    response = await test_app.patch(
        f"/api/companies/{seed_company['company_id']}", headers=headers, json=data
    )
    ...
    updated_company = await Company.filter(
...
async def test_view_company(test_app: AsyncClient, jwt_token_admin, seed_company):
    ...
    response = await test_app.get(
        f"/api/companies/{seed_company['company_id']}", headers=headers
    )
...
async def test_delete_company(test_app: AsyncClient, jwt_token_admin, seed_company):
    ...
    deleted_company = await Company.filter(
        company_id=seed_company["company_id"]
    ).first()
...
async def test_get_companies(
    test_app: AsyncClient, jwt_token_admin, seed_company, seed_relation
):
    ...
    assert any(
        company["company_id"] == seed_company["company_id"] for company in companies
    )
