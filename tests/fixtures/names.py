...
async def seed_role_admin():
    role = await UserRole.create(role_name="admin")
    return {"role_id": str(role.role_id), "role_name": role.role_name}
...
async def seed_permission():
    permission = await Permissions.create(
        permission_id="test_permission", permission_name="Тестовое разрешение"
    )
    return {
...
async def seed_role_permission_relation(seed_role_admin, seed_permission):
    ...
    permission = await Permissions.get_or_none(
        permission_id=seed_permission["permission_id"]
    )
...
async def seed_role_manager():
    role = await UserRole.create(role_name="manager")
    return {"role_id": str(role.role_id), "role_name": role.role_name}


@pytest.fixture(scope="function")
async def seed_role_user():
    role = await UserRole.create(role_name="user")
    return {"role_id": str(role.role_id), "role_name": role.role_name}
...
async def seed_legal_entity_type():
    ...
    entity_type = await LegalEntityType.create(
        legal_entity_type_id="ooo", entity_name="ООО"
    )
    return {
...
async def seed_contract_status():
    ...
    contract_status = await ContractStatus.create(
        contract_status_id="active", status_name="Active"
    )
    return {
...
async def seed_contract_status_updated():
    """Создает тестовый статус контракта."""
    contract_status = await ContractStatus.create(
        contract_status_id="waiting", status_name="Waiting"
    )
    return {
        "contract_status_id": str(contract_status.contract_status_id),
        "status_name": contract_status.status_name,
