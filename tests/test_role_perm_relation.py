...
async def test_add_role_permission_relation(
    ...
    relation = await RolePermissionRelation.filter(
        role_permission_id=response_data["role_permission_id"]
    ).first()
...
async def test_get_role_permission_relation(
    ...
    assert (
        response.json()["role_permission_id"]
        == seed_role_permission_relation["role_permission_id"]
    )
...
async def test_update_role_permission_relation(
    ...
    assert (
        response_data["role_permission_id"]
        == seed_role_permission_relation["role_permission_id"]
    )

    # ✅ Проверяем в БД
    relation = (
        await RolePermissionRelation.filter(
