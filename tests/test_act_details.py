...
async def test_view_act_detail(test_app: AsyncClient, jwt_token_user, seed_act_detail):
    ...
    assert response_data["act_detail_id"] == str(
        seed_act_detail["act_detail_id"])
...
async def test_delete_act_detail(test_app: AsyncClient, jwt_token_user, seed_act_detail):
    ...
    assert act_detail is not None, "Ошибка: act_detail не существует в БД перед удалением"
    ...
    deleted_act_detail = await ActDetails.filter(act_detail_id=seed_act_detail["act_detail_id"]).first()
