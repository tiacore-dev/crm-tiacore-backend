...
async def test_add_act(
    test_app: AsyncClient, jwt_token_admin, seed_contract, seed_company
):
...
async def test_edit_act(test_app: AsyncClient, jwt_token_admin, seed_act):
    ...
    response = await test_app.patch(
        f"/api/acts/{seed_act['act_id']}", headers=headers, json=data
    )
...
@pytest.mark.asyncio
async def test_view_act(
    test_app: AsyncClient, jwt_token_admin, seed_act, seed_act_detail
):
    """Тест просмотра информации об акте."""
    headers = {"Authorization": f"Bearer {jwt_token_admin['access_token']}"}

    response = await test_app.get(f"/api/acts/{seed_act['act_id']}", headers=headers)
...
async def test_delete_act(test_app: AsyncClient, jwt_token_admin, seed_act):
    ...
    response = await test_app.delete(f"/api/acts/{seed_act['act_id']}", headers=headers)
