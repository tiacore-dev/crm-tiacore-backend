...
@pytest.mark.parametrize(
    "endpoint, model_class, payload_key",
    [
...
async def test_xss_injection(
    ...
    response = await test_app.post(endpoint, headers=headers, json=data)
    ...
    entity = await model_class.filter(
        **{
            f"{model_class.__name__.lower()}_id": response_data[
                f"{model_class.__name__.lower()}_id"
            ]
        }
    ).first()

    # Проверяем, что XSS не осталось
    assert "script" not in getattr(entity, payload_key), (
        "XSS-инъекция сохранилась в БД!"
    )
