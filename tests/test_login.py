...
async def test_login_success(test_app):
    ...
    response = await test_app.post(
        "/api/auth/login", json={"email": "Test User", "password": "123"}
    )
...
async def test_refresh_token_success(test_app, jwt_token_admin):
    ...
    response = await test_app.post(
        "/api/auth/refresh",
        data=json.dumps({"refresh_token": jwt_token_admin["refresh_token"]}),
    )
...
async def test_login_failure(test_app):
    ...
    response = await test_app.post(
        "/api/auth/login", json={"email": "test_user", "password": "wrongpassword"}
    )
