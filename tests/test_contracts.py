...
async def test_edit_contract(
    test_app: AsyncClient, jwt_token_admin, seed_contract, seed_contract_status_updated
):
    headers = {"Authorization": f"Bearer {jwt_token_admin['access_token']}"}
    form_data = {
    ...
    updated_contract = (
        await Contract.filter(contract_id=seed_contract["contract_id"])
        .first()
    ...
    if updated_contract:
        ...
        assert (
            updated_contract.status.contract_status_id
            == seed_contract_status_updated["contract_status_id"]
        )
...
async def test_view_contract(test_app: AsyncClient, jwt_token_admin, seed_contract):
    ...
    response = await test_app.get(
        f"/api/contracts/{seed_contract['contract_id']}", headers=headers
    )
...
async def test_delete_contract(test_app: AsyncClient, jwt_token_admin, seed_contract):
    ...
    deleted_contract = await Contract.filter(
...
async def test_get_contracts(test_app: AsyncClient, jwt_token_admin, seed_contract):
    ...
    assert any(
        contract["contract_id"] == seed_contract["contract_id"]
