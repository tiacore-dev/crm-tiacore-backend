...
async def test_add_entity_company_relation(
    test_app: AsyncClient, jwt_token_admin, seed_company, seed_legal_entity
):
    ...
    relation = await EntityCompanyRelation.filter(
        entity_company_relation_id=response_data["entity_company_relation_id"]
    ).first()
...
async def test_get_entity_company_relation(
    test_app: AsyncClient, jwt_token_admin, seed_entity_relation
):
    """Проверка просмотра одной связи"""
    headers = {"Authorization": f"Bearer {jwt_token_admin['access_token']}"}

    response = await test_app.get(
        f"/api/entity-company-relations/{seed_entity_relation['entity_company_relation_id']}",
        headers=headers,
    )
    assert response.status_code == 200
    assert (
        response.json()["entity_company_relation_id"]
        == seed_entity_relation["entity_company_relation_id"]
    )
...
async def test_update_entity_company_relation(
    ...
    response_data = response.json()
    assert (
        response_data["entity_company_relation_id"]
        == seed_entity_relation["entity_company_relation_id"]
    )
