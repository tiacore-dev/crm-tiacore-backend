...
async def test_add_user_company_relation(
    test_app: AsyncClient, jwt_token_admin, seed_user, seed_company, seed_role_admin
):
    ...
    relation = await UserCompanyRelation.filter(
        user_company_id=response_data["user_company_id"]
    ).first()
...
async def test_get_user_company_relation(
    test_app: AsyncClient, jwt_token_admin, seed_relation
):
    """Проверка просмотра одной связи"""
    headers = {"Authorization": f"Bearer {jwt_token_admin['access_token']}"}

    response = await test_app.get(
        f"/api/user-company-relations/{seed_relation['user_company_id']}",
        headers=headers,
    )
    assert response.status_code == 200, (
        f"Ошибка: {response.status_code}, {response.text}"
    )
    assert response.json()["user_company_id"] == seed_relation["user_company_id"]


@pytest.mark.asyncio
async def test_update_user_company_relation(
    test_app: AsyncClient, jwt_token_admin, seed_relation, seed_role_manager
):
    ...
    relation = (
        await UserCompanyRelation.filter(
    ...
    assert relation.role.role_name == "manager", f"""Роль не обновилась в БД! Ожидали 
        'manager', а получили '{relation.role.role_name}'"""
