...
async def get_permissions(
    ...
    try:
        ...
        order_by = f"{'-' if filters.get('order') == 'desc' else ''}{
            filters.get('sort_by', 'permission_id')
        }"
        ...
        permissions = (
            await Permissions.filter(query)
            .order_by(order_by)
            .offset((page - 1) * page_size)
            .limit(page_size)
            .values("permission_id", "permission_name", "comment")
        )
        ...
        return PermissionsListResponseSchema(
            total=total_count, permissions=[PermissionsSchema(**p) for p in permissions]
        )
