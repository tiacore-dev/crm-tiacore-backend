...
from app.database.models import Permissions, RolePermissionRelation, User, UserRole
from app.handlers.auth import get_cached_user_data, require_superadmin
...
async def update_role_permission_relation(
    ...
    if "permission" in update_data:
        permission = await Permissions.get_or_none(
            permission_id=update_data["permission"]
        )
        if not permission:
...
async def delete_role_permission_relation(
    role_permission_id: UUID, user_data: dict = Depends(require_superadmin)
):
    relation = await RolePermissionRelation.filter(
        role_permission_id=role_permission_id
    ).first()
...
async def get_role_permission_relation(
    role_permission_id: UUID, uuser_data: dict = Depends(require_superadmin)
):
    relation = (
        await RolePermissionRelation.filter(role_permission_id=role_permission_id)
        .prefetch_related("role", "permission")
        .first()
    )
