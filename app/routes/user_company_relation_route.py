...
from app.database.models import Company, User, UserCompanyRelation, UserRole
...
async def add_user_company_relation(
    ...
    try:
        ...
        if not perms:
            raise HTTPException(status_code=403, detail="Нет разрешений")
...
async def update_user_company_relation(
    ...
    perms = await get_company_permissions_for_user(relation.user)
    if not perms:
        raise HTTPException(status_code=403, detail="Нет разрешений")
    await save_user_to_cache(relation.user, perms)
...
async def delete_user_company_relation(
    ...
    relation = (
        await UserCompanyRelation.filter(user_company_id=user_company_id)
